              ...
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['Funcionário', 'Unidade', 'Meta', 'Vencimento', 'Batida', 'Mult.', 'Valor Base', 'Valor Final', 'Status', 'Ação'].map(h => (
                    <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {p.status === 'pendente' ? (
                        <button onClick={() => marcarPago(p.id)} disabled={salvando === p.id}
                          style={{ padding: '6px 14px', borderRadius: 6, border: 'none', background: 'var(--success)', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', opacity: salvando === p.id ? 0.5 : 1 }}>
